...
print('Решение задачи вариант №4')

def prime4(n):
    primes = {}  # в него записываются
    def _sieve(start, stop):
        massive = [i for i in range(start, stop + 1)]  # создаем исходный массив
        def __sieve(number, array):  #
            for element in array:
                massive[element - start] = 0
            if array:
        ...
        for elem in massive:
            if elem:
                composite = [i for i in range(elem * 2, stop + 1, elem)]
                __sieve(elem, composite)
